...
global gamma_0 beta_0 mu beta_e_step beta_e_last gamma_e_last gamma_e_step
...
for gamma_e=0.01:gamma_e_step:gamma_e_last
    ...
    if u<=0
           u_long(beta_e_index,gamma_e_index)=0;
       else if u>=1
               u_long(beta_e_index,gamma_e_index)=1;
           else
               u_long(beta_e_index,gamma_e_index)=u;
           end
    end
    R_0(beta_e_index,gamma_e_index)=beta_0/((1+beta_e*u_long(beta_e_index,gamma_e_index)).*(gamma_0*(1+gamma_e*(1-u_long(beta_e_index,gamma_e_index)))+mu));
    

    if u_long(beta_e_index,gamma_e_index)>0 && u_long(beta_e_index,gamma_e_index)<1   %%% for three different color in 0<u_long<1
          u_long(beta_e_index,gamma_e_index)=0.5;
    end
...
contourf(X,Y,u_long');
hold on
contour(X,Y,R_0',[1 1],'r','ShowText','off','linewidth',2)
colormap cool
view(2)
