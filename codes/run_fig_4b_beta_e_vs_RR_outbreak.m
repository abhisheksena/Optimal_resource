...
figure;
 ...
 peak_woc=max(y(:,2));         %%%%% peak without control
 ...
 for gamma_e=[0.5 1 2];        %%% particular values of gamma_e
     ...
     for beta_e=0:beta_e_step:beta_e_last
         ...
         peak=[];             %%% peak corresponding to each u from 0 to 1
         for u=0:u_step:1
             u_index=u_index+1; 
             [t,y] = ode45(@opt_resource_ode,time_interval,y0,[],u,beta_e,gamma_e);
             peak(u_index)=max(y(:,2));
         end
         peak_wc=min(peak);       %%%%% peak with control
         RR_I_max(beta_e_index)= ((peak_woc - peak_wc)/peak_woc)*100;
     end
